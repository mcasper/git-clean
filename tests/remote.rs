...
fn test_git_clean_removes_remote_branches() {
    ...
    project.batch_setup_commands(&["git checkout -b test1",
    ...
    assert!(result.stdout().contains(deleted_branch_output("test1").as_str()),
            result.failure_message("command to delete test1"));
    assert!(result.stdout().contains(deleted_branch_output("test2").as_str()),
